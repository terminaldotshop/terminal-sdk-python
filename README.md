...
[![PyPI version](https://img.shields.io/pypi/v/terminal.svg)](https://pypi.org/project/terminal/)
...
pip install git+ssh://git@github.com/stainless-sdks/terminal-python.git
```
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `terminal.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `terminal.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `terminal.APIError`.
...
except terminal.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except terminal.APIStatusError as e:
...
These methods return an [`APIResponse`](https://github.com/stainless-sdks/terminal-python/tree/main/src/terminal/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/terminal-python/tree/main/src/terminal/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
```python
import httpx
from terminal import Terminal, DefaultHttpxClient
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/terminal-python/issues) with questions, bugs, or suggestions.
