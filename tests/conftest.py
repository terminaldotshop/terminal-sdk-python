...
from terminal_shop import Terminal, AsyncTerminal, DefaultAioHttpClient
from terminal_shop._utils import is_dict
...
logging.getLogger("terminal_shop").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Terminal]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Terminal(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncTerminal]:
